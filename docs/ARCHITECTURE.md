...
|---|---|---|
| `frontend/` | React 19, Vite, TypeScript, Tailwind, shadcn/ui, TanStack Query, Zustand, React Hook Form, Zod, React Router, `graphql-ws`, `@sentry/react` | GraphQL over plain `fetch` for queries/mutations (`src/lib/graphql-client.ts`, no Apollo/urql) + a `graphql-ws` client for `onNotification` — the full auth/skills/jobs/matching/realtime feature set except Google OAuth; see "What's not wired up" below |
| `api-gateway` | Go, `gqlgen` (GraphQL), `graphql-ws` (subscriptions), `golang-jwt`/JWKS client, `sentry-go` | gRPC (out, to the 4 services) · Redis (cache/rate-limit/pub-sub) · RabbitMQ (consumes `notifications.realtime`) · Sentry (errors, only public HTTP surface with `RecoverMiddleware`) |
| `auth-service` | Go, GORM, bcrypt, `golang-jwt` (RS256), `golang.org/x/oauth2`, `sentry-go` | gRPC (in, from gateway) · Postgres (`auth` schema) · Kafka (publishes `user.registered`) · RabbitMQ (publishes `notifications.email`, `notifications.realtime`) |
| `users-service` | Go, GORM, `sentry-go` | gRPC (in) · Postgres (`users` schema) · Kafka (publishes `user.skills.updated`) |
| `skills-service` | Go, GORM, Redis client, `sentry-go` | gRPC (in) · Postgres (`skills` schema) · Redis (cache) · Kafka (publishes `skill.updated`) |
| `jobs-service` | Go, GORM, Redis client, Kafka client (`franz-go`), RabbitMQ client, `sentry-go` | gRPC (in) · Postgres (`jobs` schema, incl. its own `user_skill_snapshot`/`job_matches` projections) · Redis (cache) · Kafka (publishes + 3 consumers: snapshot projection, matching worker, cache invalidation) · RabbitMQ (publishes `notifications.realtime`) |
| `notification-service` | NestJS, `amqplib`, plain `pg` (no ORM), `@sentry/node` | RabbitMQ (consumes `notifications.email` only) · Postgres (`notifications` schema) · Sentry (local/`kind` value only — not part of the production deploy, see below) |
| Shared Go foundation | `internal/platform/{logger,requestid,health,shutdown,db,jwks,cache,kafka,rabbitmq,sentry}` | Every Go service builds on this instead of reimplementing it per-service |
...
- **An optional external caller keeps that production deployment warm**: `infra/aws/keepalive-lambda/` (CloudFormation + Lambda + CloudWatch, not part of this app's own request path or diagram above) pings `/readyz` every 10 minutes from outside — see `docs/DECISIONS.md`'s "AWS keep-alive Lambda" section.
...
- **Production error visibility was a real gap, now closed by Sentry** — before this, a crash in `cmd/allinone` or a frontend render error had no signal beyond a user report; the Prometheus/Loki/Jaeger stack never runs in production (see the point above and `docker/docker-compose.observability.yml`). See `docs/DECISIONS.md`'s Sentry section. What Sentry *doesn't* replace: there's still no request tracing or log aggregation in production — Sentry captures errors and log-level events, not the full "every request's structured logs, correlated by request ID, across every hop" picture `docs/DECISIONS.md`'s centralized-logging section describes for local/`kind`.
...
- **Frontend**: Vitest + React Testing Library now configured (`vite.config.ts`'s `test` block, `npm test`/`npm run test:coverage`) — see `frontend/README.md`'s Testing section. 11 tests exist across 3 files (`lib/jwt.ts`, `lib/storage/recentlyViewedJobs.ts`, `features/auth/LoginPage.tsx`), all passing; overall coverage is still low (~13% statements) since most components/hooks remain untested — this closes the "no test tooling" half of the gap, not the "everything's covered" half.
