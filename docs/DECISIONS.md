...
## Redis Streams: notification history (a learning-driven addition)

Added specifically to give Redis Streams a real, working example in this codebase (a deliberate learning goal, not something a product requirement drove) — and it happens to close a genuine gap `frontend/README.md` had already flagged: `onNotification` is a pub/sub subscription, and **Redis pub/sub has no memory** — a `PUBLISH` with no live `SUBSCRIBE` at that exact instant is simply lost forever (already documented in `internal/gateway/realtime`'s package comment as a deliberate, accepted characteristic of the live-delivery design). That means every notification that happened before a client opened its WebSocket — including the very common case of "I registered, then the page reconnected" — was gone. A Stream is Redis's answer to "log this so someone can ask what they missed," which pub/sub structurally cannot do.

**Why a Stream instead of just a Redis List (`LPUSH`/`LRANGE`) or a Sorted Set** — the two other obvious "recent items" primitives: a Stream is purpose-built for this exact shape (an append-only, auto-ID'd log with a `MAXLEN` trim knob built into the same command that appends), so `XADD ... MAXLEN ~ 50 ...` does the append-and-trim in one round trip instead of a List's separate `LPUSH` + `LTRIM`. It's also the more transferable thing to have learned: Streams are the primitive behind consumer groups (`XREADGROUP`), which is what a real "replay-safe" event system would build on — this feature only uses the reader half (`XADD`/`XREVRANGE`), not consumer groups, since a single "give me my own recent history" reader has no competing-consumers problem to solve, but it's the natural next thing to reach for if this project ever wanted, say, multiple gateway replicas coordinating who processes what.

**Where it lives**: `internal/platform/cache`'s new `Stream` interface (`AppendNotification`/`RecentNotifications`, implemented via `XAdd`/`XRevRangeN`) sits alongside the existing `Cache`/`RateLimiter`/`PubSub` interfaces on the same `*cache.Client` — same file, same degrade-gracefully convention (a disabled Client makes `AppendNotification` a silent no-op and `RecentNotifications` report unavailable, never a panic or a failed request). `internal/gateway/realtime.Bridge` — already the one place every realtime notification passes through on its way from RabbitMQ to Redis pub/sub — now also appends each one to `UserStreamKey(userID)` in the same `Handler` call, right next to the existing `Publish`. A new `notificationHistory` GraphQL query (`schema.resolvers.go`) reads it back via `XREVRANGE` (newest first), and the frontend's `useNotifications` hook fetches it once on mount to seed the bell before the live subscription takes over.

**Trimming is approximate (`MAXLEN ~`), not exact**, matching Redis's own recommended default: exact trimming (`MAXLEN =`) has to walk and evict entries one at a time; approximate trimming lets Redis evict whole internal radix-tree nodes at once and only guarantees the stream stays "around" the cap rather than at exactly 50 — the standard production trade-off, and irrelevant here since nothing depends on the count being exactly 50 rather than roughly 50.

### Verification performed

Registered a fresh user and confirmed `notificationHistory` already contained the registration's own `welcome` notification — proving the append happens even with zero WebSocket subscribers connected (pub/sub's exact failure mode, now covered). Added a skill and posted a matching job, confirmed the resulting `job_match` notification appeared in `notificationHistory` ordered newest-first ahead of the earlier `welcome` entry (a real, non-trivial ordering assertion, not just "a list came back non-empty"). Then, with a live `graphql-ws` client subscribed, triggered a third matching job and confirmed **both** paths fired from the exact same event: the subscriber received the live push, and the very next `notificationHistory` query showed all three notifications in the correct order — proving pub/sub and the Stream are two independent, correctly-coexisting reads of the same underlying `Bridge.Handler` call, not one replacing the other. Inspected the raw Redis entry directly (`XRANGE`) to confirm the on-the-wire shape (one `payload` field holding the same JSON `Publish` sends). `go build ./...` and `go test ./...` both clean across the whole backend, including an extended `bridge_test.go` that now asserts the stream append alongside the existing pub/sub assertion, and a `cache_test.go` compile-time check that `*Client` satisfies `Stream`/`RealtimeStore`. `npm run build`/`npm run lint` clean on the frontend side. Test job postings and the test user's stream key were cleaned up afterward.

## Telemetry: Prometheus metrics on api-gateway (also a learning-driven addition)

Closes the smaller half of a scope cut this project's own plan named explicitly: "Grafana+Loki/OpenTelemetry stay optional stretch, correlation IDs are the non-optional baseline." Metrics (counters/histograms of *what happened*, scraped periodically) and distributed tracing (OpenTelemetry spans following one request across every service) are both under that "optional stretch" umbrella but are genuinely different tools — metrics needed nothing beyond a Go library and one HTTP endpoint to be immediately useful (curl `/metrics`, see real numbers), while tracing only pays off with a collector (Jaeger/Tempo) actually running to view the spans in. This adds the smaller, self-contained half; tracing remains deferred, for the same reason it always was.

**What's exported**, all via `github.com/prometheus/client_golang` (the reference Go client — no home-grown metrics type):
- `graphql_requests_total{operation,type,status}` — a counter, incremented once per GraphQL operation gqlgen finishes executing.
- `graphql_request_duration_seconds{operation,type}` — a histogram of how long each operation took, with buckets tuned for this app's actual scale (local gRPC calls resolving in low milliseconds) rather than Prometheus's HTTP-handler-oriented default buckets, which top out at 10s and would put every real request in this app in the same bottom bucket.
- `websocket_connections_active` — a gauge, the number of currently-open, successfully-authenticated `onNotification` connections.
- The standard Go runtime and process collectors (`go_goroutines`, `go_memstats_*`, `process_*`), registered explicitly rather than assumed — see below.

**Where it lives**: `internal/platform/telemetry` defines `Metrics` (the three custom metrics above) and a `graphql.HandlerExtension` + `graphql.ResponseInterceptor` (the same gqlgen extension mechanism `extension.Introspection{}`/`extension.AutomaticPersistedQuery{}` already use via `srv.Use(...)` — this is a hand-written extension using that identical mechanism, not a new one). `cmd/api-gateway/main.go` (and `cmd/allinone/main.go`, which duplicates api-gateway's GraphQL-server setup for the same reason its `wsInitFunc` copy already existed — see that file's own comment) constructs a **fresh `prometheus.NewRegistry()`**, not `prometheus.DefaultRegisterer` — deliberately, so this process's metrics are exactly what this codebase defines plus the Go/process collectors registered explicitly, not whatever any imported library might have silently registered into the global default. `/metrics` is served via `promhttp.HandlerFor` on the same mux as `/healthz`/`/readyz`/`/query`.

**The WebSocket gauge needed its own increment/decrement wiring**, not just the GraphQL extension, since gqlgen's extension chain only wraps request/response execution, not the WebSocket connection lifecycle itself. `wsInitFunc` (already the place `onNotification`'s auth happens) increments the gauge only on a *successful* auth, and stashes a private context key (`wsCountedKey`) marking that this connection was counted; the new `wsCloseFunc` (`transport.Websocket.CloseFunc`, unused until now) decrements the gauge only if that key is present. This asymmetric-looking design is deliberate: an upgrade rejected at `connection_init` (bad/missing token) never increments the gauge, and `CloseFunc` fires for *every* closed connection including rejected ones — without the marker, a rejected connection's close would decrement a gauge it never incremented, eventually driving the count negative.

### Verification performed

Registered a user, ran a valid query, a valid-but-erroring query (an unknown field), and confirmed `/metrics` showed exactly `graphql_requests_total{operation="anonymous",status="ok",type="query"} 2` and `...status="error"...} 1` — the error path genuinely distinguished from success, not just "some counter went up." Confirmed `go_goroutines` and other standard collector output appears, proving the explicit `collectors.NewGoCollector()`/`NewProcessCollector()` registration (not assumed from importing the package). Opened a real `graphql-ws` connection (the same authenticated client script used for the WebSocket-fix and Redis Streams verifications) and watched `websocket_connections_active` go `0 -> 1` while it was open and back to `0` the moment it disconnected — the actual gauge lifecycle, not just "a metric exists." `go build ./...`, `go vet ./...`, and `go test ./...` all clean across the whole backend, including a new `telemetry_test.go` covering the success/error/anonymous-operation-name cases and the gauge's inc/dec pairing via `prometheus/testutil`.

## RBAC: a role claim, and createSkill's admin gate

Closes a gap this project had named honestly in three separate places rather than quietly ignored: `docs/SECURITY.md` tracked "no role/authorization system" as an open item, `jwks/token.go`'s own `Claims` doc comment said role-based claims were "deferred until a later phase actually needs authorization decisions beyond 'is this a valid token,'" and `schema.graphqls`/`schema.resolvers.go` repeatedly noted `createSkill`/`createJob` were unauthenticated "since there's no role system yet." This is that later phase.

**Scope, deliberately kept small**: two roles (`user`, `admin`), one gated operation (`createSkill`). Not a general permissions/ACL framework — a flat two-value enum on the credential row is proportionate to what this app actually needs authorized, and generalizing further (a roles table, per-resource permissions, groups) would be solving a problem this project doesn't have yet. `createSkill`, not `createJob`, was picked as the one example: skill-taxonomy management is a genuinely admin-shaped action (a small, shared, curated vocabulary everyone else's data references), where job postings are closer to normal user content — gating both would have made the "why this one, not that one" line arbitrary instead of principled.

**Where role lives, end to end**:
- `auth.credentials.role` (migration `00003_add_role_to_credentials.sql`), `TEXT NOT NULL DEFAULT 'user'` — but every insert in Go code sets it explicitly (`Credential{..., Role: RoleUser}`) rather than relying on that DB default, since GORM sends every field's zero value on `Create`, which would silently override a DB default with `""` if the Go struct field were left unset. The DB default exists anyway as a second line of defense for any future direct-SQL insert that bypasses GORM entirely.
- The JWT's `role` claim (`jwks.Claims.Role`, set by `jwks.Sign`'s now five-argument signature) — signed by auth-service, verified by the gateway against the same JWKS as every other claim, never independently re-checked against the database per request. This is the same trust model as `sub`: once the gateway has verified the token's signature, it trusts every claim inside it, including role, for that token's remaining lifetime (up to `DefaultTTL`, 1 hour).
- `authctx.Role(ctx)` (paired with the existing `authctx.UserID(ctx)`, both set together by one `NewContext` call in `Middleware`/`wsInitFunc`) — the gateway-side read of that claim.
- `requireAdmin` (`internal/gateway/graph/helpers.go`, alongside `requireUserID`) — the one place a resolver actually makes an authorization decision. Deliberately returns a different message ("admin role required") than "authentication required" for an authenticated non-admin — this is feedback for a legitimate account holder about what they're missing, not a security boundary where distinguishing the two would leak anything an attacker doesn't already know from trying both.

**Bootstrapping the first admin is deliberately manual, not a feature**: `UPDATE auth.credentials SET role = 'admin' WHERE email = '...'`, run directly against Postgres. No self-service "become an admin" path exists or should — a real invite/promotion flow (who can promote whom, an audit trail, revocation) is a genuinely separate feature this project doesn't need to build to demonstrate RBAC's actual mechanism. A promoted admin's *existing* token still carries the old role until they log in again — roles are baked into the token at issuance, not re-read live — which is expected JWT behavior, not a bug, and was confirmed during verification below.

**Frontend**: `lib/jwt.ts`'s `decodeJwtRole` (mirroring the existing `decodeJwtSubject`, same "the gateway's AuthPayload doesn't return this, so read it from the token" reasoning) feeds `auth-store.ts`'s new `role` field, purely for **UI convenience** — `SkillsPage`'s "Add a skill" form only renders for `role === "admin"`, showing a plain explanation otherwise. This client-side check is never the actual security boundary (`requireAdmin` on the backend is); it just avoids showing a form that would predictably fail, the same reasoning `ProtectedRoute` already applies to `myProfile`.

### Verification performed

Registered a fresh user and confirmed `createSkill` was rejected with `"admin role required"` — a distinct message from the separately-confirmed unauthenticated case (`"authentication required"`), proving the two failure modes are genuinely different code paths, not the same generic rejection. Promoted that user directly via SQL, logged in again to get a token carrying the new role (confirmed the *old* token's cached role, as expected, would not have worked — not itself re-tested, since it's inherent JWT behavior already relied on elsewhere in this codebase), and confirmed `createSkill` then succeeded and returned the created skill. `go build ./...`, `go vet ./...`, and `go test ./...` clean across the whole backend, including new tests in `internal/gateway/graph/helpers_test.go` covering all three `requireAdmin` outcomes (no caller, authenticated non-admin, authenticated admin) and existing `jwks`/`authctx`/`ratelimit` tests updated for the new `Sign`/`VerifyToken`/`NewContext` signatures. `npm run build`/`npm run lint` clean on the frontend. Test skill data cleaned up afterward.

## Accessibility (WCAG)

A real pass across the existing UI, not a bolted-on audit page — every change below is either a fix to an existing component or a small addition (the skip link) that every page already benefits from, since they all render through the one shared `Layout`.

- **Skip link (WCAG 2.4.1, Bypass Blocks)**: a visually-hidden-until-focused link at the very top of `Layout`, jumping to `<main id="main-content" tabIndex={-1}>`. A keyboard or screen-reader user no longer has to tab through the full header nav on every single page.
- **`aria-current="page"` on the active nav link** (`Layout.tsx`'s `NavLink`) — the active state was already shown visually (color + underline); this makes it available non-visually too, rather than relying on color alone (WCAG 1.4.1, Use of Color).
- **The notification bell** (`NotificationsBell.tsx`) gained: `aria-haspopup`/`aria-expanded` on the trigger button so its state is announced; `role="menu"`/`role="menuitem"` on the dropdown and its items; an Escape key handler that closes the panel and returns focus to the trigger (WCAG 2.1.2-adjacent — a keyboard user always has an obvious, working way out of an open panel); and a visually-hidden `aria-live="polite"` region announcing the current unread count, so a screen-reader user gets the same "you have new notifications" signal a sighted user gets from the badge (WCAG 4.1.3, Status Messages).
- **`prefers-reduced-motion: reduce`** (`index.css`) collapses every animation/transition duration to effectively zero, app-wide, for a user who's told their OS they don't want non-essential motion (WCAG 2.3.3). Applied broadly (`*, *::before, *::after`) rather than per-component, since nothing in this app's animations carries information that would be lost by removing it — it's all polish.
- **A real page `<title>`** (`index.html`, was the Vite scaffold default "frontend") — a small thing, but the page's `<title>` is literally the first landmark a screen-reader user hears, and "frontend" told them nothing about what they'd navigated to.
- Already in place before this pass, verified rather than assumed: `lang="en"` on `<html>`, focus-visible ring styles on every interactive shadcn component, and every form field labeled via `<FormLabel>`'s `htmlFor` (gqlgen/shadcn's own convention, not something this pass added).

### What this doesn't cover (honest gaps, not claimed)

No automated accessibility testing (axe-core, Lighthouse CI) is wired into this project — every change above was reasoned through and manually verified against WCAG success criteria, not machine-checked. Color contrast wasn't independently audited beyond trusting shadcn's own neutral-palette design tokens. No screen-reader was actually used end-to-end to confirm the experience (no such tool available in this environment) — the ARIA attributes and live region are implemented to the spec's letter, not confirmed against a real assistive-technology session.

### Verification performed

`npm run build` and `npm run lint` both clean after every change. `sr-only`/`focus:not-sr-only` compiled into real CSS rules in the production build output (grepped the built CSS to confirm the utility classes weren't silently no-ops under this project's Tailwind v4 setup). No live browser/screen-reader session was available to click through the skip link or trigger the live region directly — see "what this doesn't cover" above.

## Express vs. Fastify — a standalone comparison, not a swap

`backend/express-vs-fastify/` — see its own README for the full writeup and real benchmark numbers (Fastify: ~3x the throughput, ~3.3x lower latency on the identical route, on this machine). Worth noting here only for the scoping decision: `notification-service` stays on NestJS's default Express adapter, completely untouched. NestJS's adapter pattern means Fastify could be swapped in with almost no code change (`@nestjs/platform-fastify`), but `notification-service`'s actual job is consuming RabbitMQ messages, not serving high-throughput HTTP — the performance difference this comparison demonstrates is close to irrelevant to what that service does, so an isolated, runnable comparison was the honest way to show the difference without conflating it with an unnecessary change to a service that already works fine.

## Distributed tracing (OpenTelemetry + Jaeger)

The piece `internal/platform/telemetry`'s own package doc comment named explicitly as deferred: metrics tell you "how many, how fast, in aggregate"; tracing tells you "for this *one* request, which service did it visit, in what order, how long did each hop take." The correlation ID (`internal/platform/requestid`) already lets you grep every log line for one request across services; a trace gives the same idea as a visual waterfall of spans instead of text.

**Library**: the reference `go.opentelemetry.io/otel` SDK, not a vendor-specific client — this is the actual point of OpenTelemetry existing (the same instrumentation works whether the collector on the other end is Jaeger, Grafana Tempo, or a paid SaaS; nothing here is Jaeger-specific except the exporter's endpoint). `internal/platform/tracing.InitTracerProvider` sets up a batching OTLP/gRPC exporter and registers it as the global tracer provider — same degrade-gracefully convention as every other optional dependency: an unset `OTEL_EXPORTER_OTLP_ENDPOINT` leaves OpenTelemetry's own no-op tracer in place, logged once, never a failed startup.

**Where spans actually come from, without hand-writing them per resolver**: `otelgrpc.NewClientHandler()` on every outgoing gRPC connection (api-gateway → auth/skills/users/jobs, and `allinone`'s shared `dialLoopback` helper) and `otelgrpc.NewServerHandler()` on every gRPC server (`skills-service` first, as the concrete proof; the same one-line addition applies to the other three) — both are `grpc.StatsHandler`s, the same interception point gRPC itself uses for its own internal stats, so no interceptor chain or per-method wiring was needed. `otelhttp.NewHandler` wraps the gateway's `/query` mux entry as the outermost layer, starting the root span every downstream gRPC client span attaches to — a trace with no root span is just disconnected spans, not a trace. The W3C Trace Context propagator (`otel.SetTextMapPropagator(propagation.TraceContext{})`, set once in `InitTracerProvider`) is what actually carries the trace ID across the gRPC hop as metadata; without registering it, otelgrpc's client and server handlers would each start their own disconnected trace instead of one continuing the other's.

**Why `skills-service` first, not all four**: the same "one concrete example, not a mechanical sweep" reasoning as the RBAC work picking `createSkill` over `createJob` — proving the propagation mechanism works across one real process boundary is what matters; repeating the identical one-line `grpc.StatsHandler` addition on `auth-service`/`users-service`/`jobs-service` is mechanical, not a new thing to learn, and left as an equally-easy follow-up.

### Verification performed

Brought up Jaeger (`docker/docker-compose.observability.yml`, OTLP gRPC on `4317`), ran `skills-service` and `api-gateway` with `OTEL_EXPORTER_OTLP_ENDPOINT=localhost:4317`, and drove a real `{ skills { id name } }` query through the gateway. Queried Jaeger's own HTTP API directly (`GET /api/traces?service=api-gateway`) rather than assuming from a UI screenshot (no browser available in this environment) and got back **one trace ID containing three spans across two processes**: `[api-gateway] POST /query` (the root, 42ms) as parent of `[api-gateway] skills.v1.SkillsService/ListSkills` (the client span, 24ms) as parent of `[skills-service] skills.v1.SkillsService/ListSkills` (the server span, in the *other* process, 22ms) — proving the W3C trace-context propagation genuinely worked across a real OS-process boundary, not just that spans were created. `go build ./...`, `go vet ./...`, and `go test ./...` clean across the whole backend, including a new `tracing_test.go` covering the no-endpoint-configured degrade path.

## Centralized logging (Loki + Promtail) and a Grafana dashboard over all three pillars

Every service already logs structured JSON (zap) — this closes the "but it's not *centralized*" gap: finding every log line for one request across five processes meant `grep`-ing five different files by hand. Chosen pairing: **Loki + Promtail**, the standard free/local counterpart to Prometheus (same underlying label-based query model, same company, designed to sit next to it) rather than reaching for the heavier ELK/OpenSearch stack — this project already has no need for full-text search across arbitrary unstructured logs, just "show me this service's/this request's JSON lines in order," which Loki's label-indexed (not full-text-indexed) model is specifically optimized for and cheaper to run than Elasticsearch's approach.

**How Promtail finds the logs, given this project's actual local-dev shape**: every backend service in this session runs as a bare host process (`go run`/a built binary), not inside a container — there's no Docker container log stream for Promtail's usual service-discovery mode to tail. Local-dev verification passes already redirect each service's stdout to `/tmp/<service>.log` (see `docs/RUNNING_LOCALLY.md`), so `docker/observability/promtail-config.yml` mounts `/tmp` read-only into the Promtail container and tails `*.log` directly — the practical equivalent of container-log tailing for this project's "bare processes, not containers" local shape. A real containerized deployment would instead use Promtail's Docker/Kubernetes service discovery, no config rewrite needed beyond swapping this one `scrape_configs` block.

**Grafana, provisioned not clicked-through**: `docker/observability/grafana-datasources.yml` pre-registers Prometheus, Loki, *and* Jaeger as datasources on first boot (`isDefault: true` on Prometheus) — no manual "add data source" step. Anonymous admin access (`GF_AUTH_ANONYMOUS_ENABLED`) is a deliberate choice for what's explicitly a local-only, throwaway dev dashboard, not a real deployment concern to secure.

**Why one new compose file (`docker/docker-compose.observability.yml`), not folded into the existing ones**: `docker-compose.infra.yml`/`docker-compose.yml` bring up this project's actual application dependencies and services; Jaeger/Loki/Promtail/Prometheus/Grafana *observe* those from the outside — a genuinely separate concern that shouldn't have to come up or tear down in lockstep with the app itself (you might want the app running without the observability stack, or vice versa, far more often than you'd want both forced together).

### Verification performed

All five containers (`jaeger`, `loki`, `promtail`, `prometheus`, `grafana`) came up healthy via `docker compose -f docker/docker-compose.observability.yml up -d`. Queried Prometheus's own API (`GET /api/v1/targets`) and confirmed `host.docker.internal:8080/metrics` (api-gateway) shows `health: "up"` — a real scrape succeeding, not just "the container started." Queried Loki's API directly (`GET /loki/api/v1/query_range?query={job="skillbridge"}`) and got back real, current log lines matching the exact JSON content in the live `api-gateway.log` file, and separately queried Loki's label values (`GET /loki/api/v1/label/filename/values`) to confirm log files from every backend service (not just one) were being ingested — genuinely *centralized*, not a single-source demo. Queried Grafana's own API (`GET /api/health` → `"database": "ok"`, `GET /api/datasources`) and confirmed all three datasources (Prometheus, Loki, Jaeger) were present and correctly provisioned, without ever loading the UI in a browser (none available in this environment) — API-level verification standing in for a UI screenshot, the same substitute used throughout this session wherever a live browser wasn't available.

## Frontend testing: Vitest + React Testing Library

Closes the "no test tooling configured" half of a gap `docs/ARCHITECTURE.md`/`frontend/README.md` had both named honestly since the frontend work began — not the "everything's covered" half, which stays genuinely open (see below).

**Vitest, not Jest**: this project already runs on Vite, and Vitest reuses Vite's own config, plugins, and transform pipeline directly — `vite.config.ts`'s new `test` block (via `defineConfig` imported from `"vitest/config"`, which merges Vite's and Vitest's config types into one file) means the dev server and the test runner share one config, not two that can silently drift apart the way a separate Jest+Babel setup would. This is also the current default recommendation for a Vite-based React app specifically because of that integration, not an arbitrary pick. Paired with **React Testing Library** (query by what a user would actually see — label text, role — not a component's internal state or props) + `@testing-library/user-event` (realistic keystroke/click event sequences, not a single synthetic `fireEvent`) + `@vitest/coverage-v8` (V8's own native code coverage instrumentation, no separate Istanbul compile step).

**No `globals: true`**: every test file imports `describe`/`it`/`expect` explicitly from `"vitest"`, matching this codebase's existing no-ambient-magic convention (nothing else here relies on auto-injected globals either). The one real cost of that choice: React Testing Library's own auto-cleanup-between-tests normally hooks itself into whatever global `afterEach` the test framework provides, and silently never fires without one — `src/test/setup.ts` registers it explicitly instead, which is the direct fix, not a workaround.

**A real bug this surfaced, not a test-only artifact**: writing the `LoginPage` test (submit an invalid email, expect Zod's error message) revealed that `type="email"` on the input let the *browser's own* HTML5 constraint validation intercept the form submit before React Hook Form/Zod ever ran — jsdom enforces this the same way a real browser does. Both `LoginPage` and `RegisterPage` now set `noValidate` on their `<form>`, making Zod's messages authoritative and consistent across browsers instead of whatever native validation UI each browser happens to show.

**What's actually tested, chosen for being genuinely representative rather than easiest**: `lib/jwt.ts` (pure-function claim decoding, including the "role missing → defaults to user" fallback path), `lib/storage/recentlyViewedJobs.ts` (sessionStorage-backed dedupe/cap/ordering logic), and `features/auth/LoginPage.tsx` (a full render → fill in → submit → assert cycle, covering the validation-error path, the successful-login-decodes-the-token path, and the server-error-message path — mocking only the one network call, not the component's own logic).

### Verification performed

`npm test` — all 11 tests pass. `npm run test:coverage` produced a real coverage report (`frontend/coverage/`, html + text reporters): `lib/jwt.ts` and `features/auth/LoginPage.tsx` both at 100% statement coverage, `lib/storage/recentlyViewedJobs.ts` at 90.9%, overall project coverage ~13.27% statements — an honest number, not inflated, since only 3 files were actually targeted. `npm run build` and `npm run lint` both clean afterward (the `Buffer` fix in `test/makeToken.ts` — switched to browser-native `btoa`, since this project's tsconfig is browser-only and has no Node types — and adding `coverage`/`frontend/coverage` to `eslint.config.js`'s ignores and `.gitignore` respectively, the same way `dist` and `notification-service/coverage` already are).

## Sentry: production error tracking and log capture

Closes a gap `docs/ARCHITECTURE.md` already named honestly: the full Prometheus/Loki/Jaeger/Grafana observability stack (`docker/docker-compose.observability.yml`) is local/`kind`-only, so production (`cmd/allinone` on Render, the frontend on Vercel) shipped with **zero** error visibility — if something broke there, the only signal was a user report. Free (Sentry's Developer plan: 5K errors/month, no card required) and, like every other optional dependency in this codebase, wired in behind an env var that degrades to a no-op when unset — this is not a "flip a switch later" feature, it's live the moment a real DSN is set, with no code change.

### One Go package, reused by all six entrypoints

`backend/internal/platform/sentry` follows every other `internal/platform` package's shape exactly (see its README): `InitFromEnv` returns a `*Handle` that's always safe to use, `SENTRY_DSN` unset means every method is a no-op logged once, never a startup failure. Two things beyond "forward errors":

- **`Handle.WrapCore` attaches a Sentry-reporting `zapcore.Core`** (via `github.com/TheZeroSlave/zapsentry`, the de facto standard zap↔Sentry bridge — no official one exists from Sentry) **alongside each service's own core**, via `log = log.WithOptions(zap.WrapCore(sentryHandle.WrapCore))` — one extra line in each `cmd/*/main.go`. Every `zap.Error`/`zap.Fatal` call already in this codebase (and there are many — every service's degrade-gracefully warning, every gRPC/HTTP handler error) now also reports to Sentry, without touching any of those call sites individually. This was a deliberate design choice over a mechanical sweep: `internal/platform/logger` didn't need to change at all, and adding a new `log.Error(...)` call anywhere in the future gets Sentry capture for free.
- **`Handle.UnaryServerInterceptor()` recovers a panicking gRPC handler unconditionally**, even on a disabled `Handle` (only the *reporting* half is gated on `Enabled()`). This was discovered while wiring Sentry in, not assumed: grpc-go does **not** recover a panicking handler on its own, and grepping this codebase's whole gRPC server chain (`grpc.ChainUnaryInterceptor(...)` in all four backend services plus `allinone`'s four embedded ones) found no existing recovery either — only gqlgen's generated GraphQL layer has its own separate one. A panic in any gRPC handler crashed the whole process; in `cmd/allinone` specifically, that's all four backend services and the gateway at once, the worst possible blast radius this codebase has. The interceptor now recovers, converts to `codes.Internal` (matching `backend/CLAUDE.md`'s error-handling pattern), and — only when Sentry is configured — reports the panic with the gRPC method tagged. A genuine reliability fix that happens to live in the Sentry package because reporting the panic is its other job.

`Handle.RecoverMiddleware` (net/http, via the official `github.com/getsentry/sentry-go/http` subpackage — `sentryhttp.New(sentryhttp.Options{Repanic: true}).Handle`) is the HTTP-layer counterpart, wired as the **outermost** layer of `api-gateway`'s and `allinone`'s public HTTP server — the only two processes in this codebase with real external exposure. `Repanic: true` deliberately preserves existing behavior: Go's own `net/http.Server` already recovers a handler panic per-connection (logs it, closes that one connection, the process keeps running) — this only adds the Sentry report on top of an already-safe path, unlike the gRPC case where nothing was recovering it at all.

### What never leaves this app: bodies, headers, cookies

Verified by reading `sentry-go`'s own `data_collection.go` rather than assumed — its documented default is to collect **every** HTTP body type verbatim unless told otherwise, while headers/cookies/query params default to a built-in case-insensitive denylist (`auth`, `bearer`, `token`, `password`, `secret`, `session`, `jwt`, ... — covers this app's `Authorization` header). Since `register`/`login` carry a plaintext password in the GraphQL POST body, `InitFromEnv` explicitly sets `ClientOptions.DataCollection.HTTPBodies` to an **empty, non-nil** slice — the one field where the SDK's default isn't already safe for this app. The frontend SDK (`@sentry/react`) doesn't have an equivalent automatic body-scrubbing default, so `frontend/src/lib/sentry.ts`'s `beforeSend` strips `event.request.data` unconditionally as the same protection on that side. See `docs/SECURITY.md`.

### Sample rate: opt-in, not Sentry's own default

`SENTRY_TRACES_SAMPLE_RATE` (backend) / `VITE_SENTRY_TRACES_SAMPLE_RATE` (frontend) both default to `0` — performance tracing off — rather than Sentry's own historical "sample everything" default. This project would rather an operator opt in deliberately (`SENTRY_TRACES_SAMPLE_RATE=0.1` in `render.yaml`, commented out) than silently burn a chunk of the free tier's performance-unit quota the first time this ships, mirroring this codebase's existing "generous defaults, but never silently expensive" stance (see the rate limiter's `DefaultConfig`).

### Frontend: an ErrorBoundary that didn't exist before

`frontend/src/App.tsx` had no error boundary at all — an uncaught render error showed a blank white page in production with nothing to investigate. `Sentry.ErrorBoundary` now wraps the whole route tree with a minimal "Something went wrong" fallback (a reload button, styled consistently with the rest of the app via the existing `Button` component); it's a no-op pass-through (renders children unchanged) when `initSentry()` never ran. Source map upload (`@sentry/vite-plugin`, `vite.config.ts`) is a separate, genuinely optional opt-in gated on `SENTRY_AUTH_TOKEN`/`SENTRY_ORG`/`SENTRY_PROJECT` all being set at **build** time — deliberately never a `VITE_`-prefixed var (those get inlined into the shipped bundle) — with `build.sourcemap` only set to `"hidden"` when that upload is actually configured, so a normal deploy with no Sentry account set up doesn't ship unreferenced `.map` files for nothing.

### notification-service: real value, but local/`kind`-only

`@sentry/node`, initialized in `main.ts` before `NestFactory.create`, with targeted `Sentry.captureException` calls at `RabbitmqConsumer`'s two real failure paths (a malformed message routed to the DLQ, an unexpected processing error) rather than a general instrumentation sweep — this service's only HTTP surface is health checks, so there's no request-layer traffic for auto-instrumentation to catch, and the RabbitMQ consumer is the one place this service's own bugs would otherwise be silent beyond a pino log line. Stated honestly: this only has value locally/in `kind` today, since `notification-service` isn't part of `render.yaml`'s production deploy (see `cmd/allinone/main.go`'s package doc comment — Kafka/RabbitMQ async flows, including this service, simply don't run in production). Wired in anyway per `CLAUDE.md`'s "every service gets shared platform behavior, not optional per-service boilerplate to skip" — and it's a five-line change once the pattern exists.

### Verification performed

`go build ./...`, `go vet ./...`, `go test ./...` all clean across the whole backend (`internal/platform/sentry` gets its own test file: disabled-`Handle` no-op behavior, an invalid `SENTRY_TRACES_SAMPLE_RATE` falling back to `0` rather than failing startup, and — the reliability fix — `UnaryServerInterceptor` recovering a panic and returning `codes.Internal` even on a disabled `Handle`). `npm run build`, `npm run lint`, `npm test` all clean on both `frontend/` (11 tests, zero regressions) and `backend/notification-service/` (19 tests, zero regressions). `npm audit fix` (non-breaking) cleared every vulnerability the two new frontend/notification-service dependencies pulled in; `notification-service`'s one remaining flagged dependency chain (`multer` → `@nestjs/platform-express` → `@nestjs/core`) is pre-existing, requires a breaking major-version bump, and was left alone as out of this change's scope.

No live Sentry project exists in this environment (an external account only the repo owner can create — see `docs/DEPLOYMENT.md`), so the one thing not verified end-to-end is a real event actually landing in a Sentry project's issue stream — the same category of gap this doc already documents for Google OAuth's live-login flow. Everything up to that point (SDK initialization, the degrade-gracefully paths on every optional-dependency check, the gRPC panic-recovery fix, the DSN-shape validation `sentry.Init` itself performs) was verified directly against the real `sentry-go`/`zapsentry`/`@sentry/react` packages' actual source and behavior, not assumed from documentation.

## AWS keep-alive Lambda (CloudFormation + Lambda + CloudWatch)

`infra/aws/keepalive-lambda/` — a CloudFormation stack combining CloudFormation, Lambda, and CloudWatch into one coherent, genuinely useful piece rather than three separate token integrations. See `infra/aws/keepalive-lambda/README.md` for the practical how-to (deploy/teardown commands, cost accounting); this section is the "why."

### The actual problem this solves, not an invented one

`docs/DEPLOYMENT.md` already documents two real free-tier failure modes this project lives with: Render's free web service spins down after ~15 minutes idle (the next request pays a full cold start of `cmd/allinone` — all four backend services plus the gateway), and Supabase's free project auto-pauses after 7 days of inactivity, needing a manual dashboard restore. Rather than three disconnected AWS integrations invented just to exercise each service once, one scheduled Lambda solves both real problems: it pings the deployed backend's **`/readyz`**, not `/healthz`. `/readyz` is the endpoint that actually pings Postgres (`backend/cmd/allinone/main.go`'s `readyCheckDB`), so one HTTP request both resets Render's idle timer (any request does that) *and* exercises a real Supabase connection recently enough to dodge its 7-day clock. This is the same "closes a genuine gap, not a product requirement drove it, but it happens to" reasoning this doc already used for Redis Streams' notification history.

### Why CloudFormation over Terraform/CDK/SAM

The user asked for CloudFormation specifically. Among CloudFormation's own flavors, plain CFN (not the SAM CLI, which is itself a CloudFormation superset with its own separate `sam` binary and `sam build`/`sam deploy` workflow) was chosen to avoid an extra tool beyond the AWS CLI this project doesn't otherwise need — the same low-extra-tooling bias `buf`'s local-plugin generation over BSR remote generation already established (`backend/CLAUDE.md`).

### Why inline `ZipFile`, not S3-packaged deployment

The handler is one file, ~900 bytes (checked directly: extracted and syntax-validated with `node --check`, well under CloudFormation's 4KB inline-code limit for `AWS::Lambda::Function`'s `Code.ZipFile`). Packaging to S3 (`aws cloudformation package` + a bucket to hold the artifact) is real, ongoing infrastructure — a bucket to create, own, and eventually clean up — for a health check with zero npm dependencies. Inline keeps the entire deploy to one `aws cloudformation deploy` command with nothing else to provision first, mirroring this project's existing bias toward fewer moving parts where one genuinely suffices (e.g. Redis as a 15-line hand-written Deployment manifest, no Helm chart, in the Kubernetes phase).

### CloudWatch Alarm math: why `DatapointsToAlarm: 2` and `Threshold: 1`, not `Threshold: 2`

A real correctness detail, not an arbitrary choice: the Lambda runs at most once per 10-minute period (`ScheduleExpression: rate(10 minutes)`), so its `Errors` metric can only ever be `0` or `1` within any single `Period: 600` window — a naive `Threshold: 2` on a single period could **mathematically never fire** at this invocation rate. The alarm is instead configured `Threshold: 1`, `EvaluationPeriods: 2`, `DatapointsToAlarm: 2` — both of the last two 10-minute periods must individually show at least one error, i.e. two consecutive failed pings (~20 minutes of sustained failure). This tolerates a single transient blip (a slow cold start, a momentary network hiccup) without paging anyone, while still catching a real outage. `TreatMissingData: notBreaching` avoids a false alarm if the schedule itself was briefly disabled — an absence of data isn't the same claim as a check that actually failed.

### IAM: least privilege, not the managed basic-execution role

The Lambda's execution role grants exactly `logs:CreateLogStream`/`logs:PutLogEvents` on its own log group's ARN, not AWS's managed `AWSLambdaBasicExecutionRole` (which grants the same two actions but scoped by wildcard to every log group matching `/aws/lambda/*` — broader than this one function ever needs). See `docs/SECURITY.md`.

### Terminology note: EventBridge, not "CloudWatch Events"

The 10-minute schedule is an `AWS::Events::Rule` — Amazon EventBridge, which is what "CloudWatch Events" was renamed to in 2019. Both CloudWatch (Logs, the Alarm) and EventBridge (the schedule) are genuinely present in this stack; called out explicitly here so it reads as a deliberate clarification of current AWS naming, not a mix-up about which service does what.

### Scope: infra practice like `k8s/`/`jenkins/`, with one real exception

Like the rest of this repo's infra-practice folders, this isn't wired into CI and needs real AWS credentials only the account owner has to deploy — see `docs/DEPLOYMENT.md`'s optional step 7. Unlike `k8s/`/`jenkins/`, though, this one **does** affect real production behavior if actually deployed (keeping the live Render/Supabase deployment warm) — worth stating plainly rather than lumping it in with infra that's purely local learning.

### Verification performed

The template was validated structurally, not just by inspection: parsed with `js-yaml` extended with every CloudFormation intrinsic function used (`!Sub`, `!Ref`, `!GetAtt`, `!If`, `!Not`, `!Equals`) to confirm well-formed YAML and that every resource/condition/output resolves to what's expected (`Resources`, `Conditions`, `Outputs` keys all printed and checked against the template's own design); the inline Lambda handler was extracted from the parsed template and checked directly with `node --check` (valid JS syntax) and its byte size measured (924 bytes, under the 4KB inline-code limit). No AWS CLI or credentials exist in this environment, so `aws cloudformation validate-template` and an actual deploy are the one thing left undone here — documented as a `docs/DEPLOYMENT.md` checklist step, the same category of gap this doc already carries for Google OAuth's live-login flow and now Sentry's live-event delivery.
