...
- [`docs/PROJECT_OVERVIEW.md`](PROJECT_OVERVIEW.md) — what the product does in plain terms: registration, skills, job postings, matching, notifications. What's real vs. simulated (no real email is sent, matching is simple overlap scoring). Register/login/profile now have a real frontend UI (see `frontend/README.md`); everything else is still exercised via the GraphQL API directly.
...
- [`docs/SECURITY.md`](SECURITY.md) — what's actually verified secure (bcrypt cost, RS256+JWKS, no SQL-injection surface, CORS, a basic RBAC gate on `createSkill`) vs. genuinely still missing (refresh-token rotation, per-resource ownership). Written against the code, not aspirational.
...
- [`backend/express-vs-fastify/README.md`](../backend/express-vs-fastify/README.md) — a standalone, isolated comparison of Express and Fastify on the identical route, with real benchmark numbers. Not used by `notification-service` (still Express via NestJS) or any other part of this project — a learning-only example.
